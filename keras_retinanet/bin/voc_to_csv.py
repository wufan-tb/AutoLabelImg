...
dataset_path='F:/Traing_folder/jiemian/jiemian_QP'
class PascalVOC2CSV(object):
    def __init__(self, xml=[], ann_path=dataset_path+'/annotations.csv', classes_path=dataset_path+'/classes.csv'):
        '''
                ...
                with open(xml_file, 'r') as fp:
                    for p in fp:
                        ...
                        if '<object>' in p:
                            ...
                            self.annotations.append(['JPEGImages/'+self.file_name, x1, y1, x2, y2, self.supercategory])#os.path.join('JPEGImages', self.file_name)
            except:
                continue
...
if __name__ == "__main__":
    xml_file = glob.glob(dataset_path+'/Annotations/*.xml')
